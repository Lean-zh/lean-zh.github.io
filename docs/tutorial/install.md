...
为了使用 Lean，你首先需要安装 [VS Code](https://code.visualstudio.com/) 和 Git。接下来你需要安装 elan，elan会自动帮你安装Lean 4 stable，接下来你就可以使用 lake 创建自己的 Lean 项目。

## Windows安装elan

last update: 2024/08/05  本段教程作者：[subfish](https://github.com/subfish-zhou)

你可以在这里获得 [VS Code](https://code.visualstudio.com/download) 和 [Git](https://gitforwindows.org/)。

当你安装完 VS Code 后，你需要安装 lean4 扩展（extension）。如果你的网络环境足够好，你可以在lean4扩展的欢迎页选择 Get Start 来完成安装，或者创建一个空的 Lean 文件时扩展会弹出提示提醒你安装 elan 和 Lean 4。
此外你还可以在cmd或powershell中运行命令
...
如果你遇到了"SSL connect error", "Timeout was reached","Failed to connect to github.com port 443"...等错误，就是说明你的网络环境不理想。如果是这样你可以通过[上交镜像源](https://s3.jcloud.sjtu.edu.cn/899a892efef34b1b944a19981040f55b-oss01/elan/mirror_clone_list.html)安装。下面演示在网络环境不理想的条件下的安装。
...
添加 elan 的安装地址到PATH环境变量（如果是默认安装，那么是 `%USERPROFILE%\.elan\bin`）。

重启终端输入 `elan --version`，如果能正常输出版本号，那么你已经装好了 elan 和 Lean 4 stable 版本。

## Linux安装elan

本段教程作者：[RexWzh](https://github.com/RexWzh)。在 Ubuntu 22.04 系统上测试通过。
...
或者在[上交镜像源](https://s3.jcloud.sjtu.edu.cn/899a892efef34b1b944a19981040f55b-oss01/elan/mirror_clone_list.html)下载，查询你自己系统对应的elan版本。
...
elan self update # 更新elan
elan show        # 显示已安装的Lean版本

# 切换默认的Lean版本，例如leanprover/lean4:v4.11.0-rc1 
# stable是最新稳定版本，所有版本可见 https://github.com/leanprover/lean4/releases
elan default leanprover/lean4:stable 

# 也可设置，只在当前目录下使用的Lean版本
elan override set leanprover/lean4:stable
```

elan 在Windows下的管理目录为 `%USERPROFILE%\.elan\bin`，在Linux下的管理目录为 `$HOME/.elan`，内容形如
...
本段教程作者：[subfish](https://github.com/subfish-zhou)
...
以创建一个名为your_project_name的空白新项目。这个项目的内容形如
...
如果你想在这个现有的工程中引用Mathlib4，你需要在`lakefile.lean`文件尾中加入
...
保存文件后VS code会提示"Restart Lean"，点不点都没关系。

下面要下载Mathlib，注意让终端路径在你的项目文件夹下。如果你的网络情况好，那么在终端中运行
```
...
否则（会相当艰难），参考[这个解决方案](https://zhuanlan.zhihu.com/p/680690436)。（本教程作者subfish不保证该参考方案的可靠性）
...
同时你的项目文件夹中出现了.lake\packages文件夹，那么证明你安装Mathlib成功了，此时"Restart Lean"即可使用。**注意：你要在项目所在的目录中运行VS code，才能让Lean使用Mathlib。**
...
*关于Mathlib的更多内容请参考[Mathlib Wiki](https://github.com/leanprover-community/mathlib4/wiki/Using-mathlib4-as-a-dependency)*
...
Mac 系统按[官网教程](https://leanprover-community.github.io/install/macos_details.html)使用 homebrew 来快速配置。